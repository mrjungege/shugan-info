---
name: shugan-info
description: 这是一个基于位置的 OpenClaw skill，用于获取商场促销信息和搜索服务。它根据用户需求自动判断模式：**promotion模式**返回促销榜单URL；**search模式**返回商场店铺URL列表，由OpenClaw用LLM进行两轮语义匹配筛选。**OpenClaw 必须严格按照主入口函数 `invoke` 调用 skill，不得自行改变指令格式。**
---
...
- `search` 模式
  ...
  - **返回数据格式**：
    ...
    ```
  - **OpenClaw 处理流程（两轮筛选）**：
    1. **第一轮筛选**：用 LLM 对每个店铺的 `shopName` 和 `urls` 键名进行语义匹配，筛选出相关店铺
    2. **第二轮筛选**：对第一轮筛选出的店铺，获取每个 URL 的动态页面文本（Puppeteer 抓取 JavaScript 渲染后的内容，建议忽略图片/CSS/字体等资源以加快处理速度），再用 LLM 对页面文本进行关键词匹配确认
    3. **返回结果**：将完成匹配的店铺信息返回给用户，格式为：`店铺名称 + URL键名 + URL地址 + 匹配文本的上下文（约150字）`
...
### search 模式
用户想要搜索具体的商品、服务或店铺，如"咖啡"、"火锅"、"美甲"等具体需求时使用。
